...
class WorkerSignupForm(SignupForm):
    first_name = forms.CharField(label=_('first_name'), max_length=50, required=True, strip=True)
    last_name = forms.CharField(label=_('last_name'), max_length=50, required=True, strip=True)
    phone = forms.CharField(label=_('phone'), max_length=15, required=True, strip=True)
...
class CompanySignupForm(SignupForm):
    company_name = forms.CharField(label=_('company_name'), max_length=100, required=True)
    address = forms.CharField(label=_('address'), max_length=250, required=True)
    phone = forms.CharField(label=_('phone'), max_length=15, required=True)
    def save(self, request):
